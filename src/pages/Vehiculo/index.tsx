...
export default function VehiculoDetalle() {
  ...
  const inputStyle: React.CSSProperties = {
    height: 42, borderRadius: 12, border: '1.5px solid rgba(20,20,19,0.12)',
    background: 'rgba(255,255,255,0.9)', padding: '0 14px', fontSize: 14,
    fontFamily: 'var(--font)', color: 'var(--ink)', outline: 'none', width: '100%',
  }

  return (
    <div style={{ height: '100svh', overflow: 'hidden', background: 'radial-gradient(ellipse 120% 60% at 60% 0%, #EDE8E0 0%, #F3F0EE 55%, #F7F4F0 100%)' }}>
      ...
      <div style={{
        ...
        borderBottom: '0.5px solid rgba(20,20,19,0.08)',
      }}>
      ...
      <div className="scrollable" style={{ height: 'calc(100svh - 56px)', padding: '16px 20px', paddingBottom: 120 }}>
        ...
        {logo && (
          <div style={{ display: 'flex', justifyContent: 'center', marginBottom: 16 }}>
            <div style={{
              ...
              background: '#fff', boxShadow: '0 2px 16px rgba(20,20,19,0.10)',
              border: `1.5px solid ${c.text}20`,
              display: 'flex', alignItems: 'center', justifyContent: 'center',
              padding: '12px 14px',
            }}>
              <img src={logo} alt={v.marca} style={{ width: '100%', height: '100%', objectFit: 'contain', filter: 'brightness(0)', opacity: 0.85 }} />
            </div>
          </div>
        )}
        ...
        {cambiandoEstado && (
          <div style={{ background: 'rgba(122,171,142,0.08)', borderRadius: 18, padding: 16, marginBottom: 16, border: '1.5px solid rgba(122,171,142,0.25)' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                <button onClick={() => setCambiandoEstado(false)} style={{ flex: 1, height: 42, borderRadius: 999, border: '1.5px solid rgba(20,20,19,0.12)', background: 'transparent', cursor: 'pointer', fontSize: 13, fontWeight: 600, color: 'var(--muted)', fontFamily: 'var(--font)' }}>Cancelar</button>
                <button onClick={handleConfirmarVenta} disabled={savingEstado || !precioVenta} style={{ flex: 2, height: 42, borderRadius: 999, border: 'none', cursor: 'pointer', background: !precioVenta ? 'rgba(20,20,19,0.1)' : '#7AAB8E', color: !precioVenta ? 'var(--muted)' : '#fff', fontSize: 13, fontWeight: 700, fontFamily: 'var(--font)' }}>
        ...
        <div style={{ background: 'rgba(255,255,255,0.85)', borderRadius: 20, padding: '4px 16px', marginBottom: 16 }}>
          ...
          {(v.gastos_adicionales ?? 0) > 0 && <Row label="Gastos adicionales" value={`−${fmt(v.gastos_adicionales!)}`} color="#C07070" />}
          {totalReparaciones > 0 && <Row label="Reparaciones" value={`−${fmt(totalReparaciones)}`} color="#C07070" />}
          <Row label="Costo total" value={fmt(costoTotal)} bold />
          {v.precio_venta && <Row label="Precio de venta" value={fmt(v.precio_venta)} />}
          {ganancia != null && <Row label="Ganancia neta" value={ganancia >= 0 ? `+${fmt(ganancia)}` : fmt(ganancia)} color={ganancia >= 0 ? '#7AAB8E' : '#C07070'} bold />}
        ...
        {/* Add rep form */}
        {addingRep && (
          <div style={{ background: 'rgba(255,255,255,0.9)', borderRadius: 20, padding: 16, marginBottom: 12, boxShadow: '0 2px 16px rgba(20,20,19,0.08)', border: '1px solid rgba(20,20,19,0.08)' }}>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              ...
              <motion.button onClick={handleSaveRep} disabled={saving || !repDesc || !repCosto} whileTap={{ scale: 0.97 }}
                style={{ width: '100%', height: 46, borderRadius: 999, border: 'none', cursor: 'pointer', background: (saving || !repDesc || !repCosto) ? 'rgba(20,20,19,0.12)' : 'var(--ink)', color: (saving || !repDesc || !repCosto) ? 'var(--muted)' : '#F3F0EE', fontSize: 14, fontWeight: 700, fontFamily: 'var(--font)' }}>
        ...
        {loadingReps ? (
          ...
          <div style={{ background: 'rgba(255,255,255,0.6)', borderRadius: 16, padding: 20, textAlign: 'center', border: '1.5px dashed rgba(20,20,19,0.10)' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {reparaciones.map((r) => {
              ...
              return (
                <div key={r.id} style={{ background: 'rgba(255,255,255,0.85)', borderRadius: 16, padding: '12px 14px', display: 'flex', alignItems: 'center', gap: 12, boxShadow: '0 1px 8px rgba(20,20,19,0.04)' }}>
                  ...
                  <div style={{ fontSize: 14, fontWeight: 800, color: '#C07070', flexShrink: 0 }}>−{fmt(r.costo)}</div>
                </div>
              )
            })}
          </div>
